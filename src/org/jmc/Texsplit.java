...
public class Texsplit
{
	 ...
	 * Private method to retrieve textures from a texturepack in a list.
	 * This was extracted into a separate method so it can be reused between the split and merge
	 * methods.
	 * 
	 * @param texturePack
	 * @param scale
	 ...
	 * @throws Exception
	 */
	private static List<Texture> getTextures(File texturePack, double scale, ProgressCallback progress) throws Exception
	{
		...
		for (int i = 0; i < fileNodes.getLength(); i++)
		{
			...
			for (int j = 0; j < texNodes.getLength(); j++)
			{
				...
				ret.add(new Texture(texName,texture));
	 ...
	 * @param scale Scaling to apply to textures.
	...
	/**
	 *
	 * Reads a Minecraft texture pack and splits the individual block textures into separate images then merges
	 * them into a single file containing all the textures.
	 * 
	 * @param destination Directory to place the output files. They will be called "texture.png" and "texture_a.png"
	 * @param texturePack A Minecraft texture pack file. If null, will use minecraft's default textures.
	 * @param scale Scaling to apply to textures.
	 * @param alphas Whether to export separate alpha masks.
	 * @param progress If not null, the exporter will invoke this callback to inform on the operation's progress.
	 * @throws Exception if there is an error.
	 */
	public static void mergeTextures(File destination, File texturePack, double scale, boolean alphas, ProgressCallback progress) throws Exception
	{
		if(destination==null)
			throw new IllegalArgumentException("destination cannot be null");
		...
		//calculate maxwidth so to keep the size of the final file more or less square
		double surface=0;
		for(Texture texture: textures)
		...
		{			
			...
			if(wmax<wused) wmax=wused;
			if(hmax<(hused+hcurr)) hmax=hused+hcurr;			
		}
		...
		for(Texture texture: textures)
		{
			...
			gtex.drawImage(texture.image, rect.x, rect.y, 
